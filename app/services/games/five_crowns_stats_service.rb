...
module Games
  class FiveCrownsStatsService
    ...
    def stats
      ...
      won_games = sessions.select { |s| s.scoresheet && @game.game_engine.leaderboard(s.scoresheet).present? && @game.game_engine.leaderboard(s.scoresheet).first[0] == @user.username }
      ...
      sessions.each do |session|
        ...
        trophies.each do |key, value|
          trophy_counts[key] += 1 if value && value[:player] == @user.username
        end
      end
